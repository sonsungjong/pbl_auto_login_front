...
async function openTargetApp() {
  ...
  try {
    ...
    const authrtId = String(data.authrt_id || '').trim();
    if (!authrtId) {
      throw new Error('서버에서 인증 코드를 받지 못했습니다.');
    }

    //const url = `${TARGET_URL}?${LAUNCH_TOKEN_QUERY}=${encodeURIComponent(authrtId)}`;
    const url = `${TARGET_URL}?${LAUNCH_TOKEN_QUERY}#${authrtId}`;
